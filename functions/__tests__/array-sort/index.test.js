...
describe("arraySort", () => {
  // primitives — numbers
  it.each(shapes)(
    "sorts numbers ascending by default (%s input)",
    async (_label, wrap) => {
      const out = await arraySort({ array: wrap([3, 1, 4, 1, 5, 9, 2]) });
      expect(out).toEqual({
        resultModel: [1, 1, 2, 3, 4, 5, 9],
        resultSchema: [1, 1, 2, 3, 4, 5, 9],
      });
    },
  );

  it.each(shapes)(
    "sorts numbers ascending (explicit) (%s input)",
    async (_label, wrap) => {
  ...
  it.each(shapes)(
    "sorts numbers descending (%s input)",
    async (_label, wrap) => {
      const out = await arraySort({
        array: wrap([3, 1, 2]),
        direction: "desc",
      });
  ...
  // primitives — strings
  it.each(shapes)(
    "sorts strings ascending (%s input)",
    async (_label, wrap) => {
      const out = await arraySort({
        array: wrap(["banana", "apple", "cherry"]),
        direction: "asc",
      });
      expect(out).toEqual({
  ...
  it.each(shapes)(
    "sorts strings descending (%s input)",
    async (_label, wrap) => {
      const out = await arraySort({
        array: wrap(["banana", "apple", "cherry"]),
        direction: "desc",
      ...
      expect(out).toEqual({
        resultModel: ["cherry", "banana", "apple"],
        resultSchema: ["cherry", "banana", "apple"],
      });
    },
  );
  ...
  // objects with path
  it.each(shapes)(
    "sorts objects by property path ascending (%s input)",
    async (_label, wrap) => {
  ...
  it.each(shapes)(
    "sorts objects by property path descending (%s input)",
    async (_label, wrap) => {
      const array = [{ age: 30 }, { age: 20 }, { age: 25 }];
      const out = await arraySort({
        array: wrap(array),
  ...
  it.each(shapes)(
    "sorts objects by string property path (%s input)",
    async (_label, wrap) => {
  ...
  it.each(shapes)(
    "sorts objects by nested path (%s input)",
    async (_label, wrap) => {
      const array = [
        { user: { score: 80 } },
        { user: { score: 60 } },
  ...
  // date sorting
  it.each(shapes)(
    "sorts date strings ascending when valueIsDate is true (%s input)",
    async (_label, wrap) => {
  ...
  it.each(shapes)(
    "sorts date strings descending when valueIsDate is true (%s input)",
    async (_label, wrap) => {
      const array = ["2023-06-15", "2021-01-01", "2022-12-31"];
      const out = await arraySort({
        array: wrap(array),
  ...
  it.each(shapes)(
    "sorts objects by date property path (%s input)",
    async (_label, wrap) => {
      const array = [
  ...
  // edge cases
  it.each(shapes)(
    "returns empty array for empty input (%s input)",
    async (_label, wrap) => {
  ...
  it.each(shapes)(
    "returns single-element array unchanged (%s input)",
    async (_label, wrap) => {
  ...
  it("throws when array is missing", async () => {
    await expect(arraySort({})).rejects.toThrow(
      "Array Sort: 'array' is required!",
    );
  });

  it("throws when array is undefined", async () => {
    await expect(arraySort({ array: undefined })).rejects.toThrow(
      "Array Sort: 'array' is required!",
    );
  });

  it("throws when array is null", async () => {
    await expect(arraySort({ array: null })).rejects.toThrow(
      "Array Sort: 'array' is required!",
    );
  });

  it("throws when array is not an array or collection-like object", async () => {
    await expect(arraySort({ array: "not-an-array" })).rejects.toThrow(
      "Array Sort: 'array' is required!",
    );
  });

  it("throws when array is a plain object without a data property", async () => {
    await expect(arraySort({ array: { foo: "bar" } })).rejects.toThrow(
      "Array Sort: 'array' is required!",
    );
  });
  ...
  it("throws when the data property is not an array", async () => {
    await expect(arraySort({ array: { data: "nope" } })).rejects.toThrow(
      "Array Sort: 'array' is required!",
    );
  });
});
