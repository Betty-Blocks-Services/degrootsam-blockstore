...
describe("mapArray", () => {
  it.each(shapes)(
    "maps array with simple path (%s input)",
    async (_label, wrap) => {
      const array = wrap([
        { name: "Alice", age: 25 },
        { name: "Bob", age: 30 },
  ...
  it.each(shapes)(
    "maps array with nested path (%s input)",
    async (_label, wrap) => {
      const array = wrap([
  ...
  it.each(shapes)(
    "maps array with numeric values (%s input)",
    async (_label, wrap) => {
      const array = wrap([
        { name: "Alice", age: 25 },
        { name: "Bob", age: 30 },
      ...
      expect(out).toEqual({
        resultSchema: [25, 30, 35],
        resultModel: [25, 30, 35],
      });
    },
  );

  it.each(shapes)(
    "maps array with boolean values (%s input)",
    async (_label, wrap) => {
      const array = wrap([
      ...
      expect(out).toEqual({
        resultSchema: [true, false, true],
        resultModel: [true, false, true],
      });
    },
  );

  it.each(shapes)(
    "maps array with mixed value types (%s input)",
    async (_label, wrap) => {
      const array = wrap([
      ...
      expect(out).toEqual({
        resultSchema: [95, 87, 72],
        resultModel: [95, 87, 72],
      });
    },
  );

  it.each(shapes)(
    "maps array with deeply nested path (%s input)",
    async (_label, wrap) => {
      const array = wrap([
      ...
      expect(out).toEqual({
        resultSchema: ["dark", "light", "dark"],
        resultModel: ["dark", "light", "dark"],
      });
    },
  );

  it.each(shapes)(
    "maps array with null values (%s input)",
    async (_label, wrap) => {
      const array = wrap([
        { name: "Alice", age: 25 },
        { name: "Bob", age: null },
      ...
      expect(out).toEqual({
        resultSchema: [25, null, 35],
        resultModel: [25, null, 35],
      });
    },
  );

  it.each(shapes)(
    "maps array with undefined values (%s input)",
    async (_label, wrap) => {
      const array = wrap([
        { name: "Alice", age: 25 },
      ...
      expect(out).toEqual({
        resultSchema: [25, undefined, 35],
        resultModel: [25, undefined, 35],
      });
    },
  );
  ...
  it.each(shapes)(
    "maps single element array (%s input)",
    async (_label, wrap) => {
  ...
  it.each(shapes)(
    "handles path that doesn't exist on some objects (%s input)",
    async (_label, wrap) => {
      const array = wrap([
        { name: "Alice", age: 25 },
        { name: "Bob" },
  ...
  it.each(shapes)(
    "handles path that returns undefined (%s input)",
    async (_label, wrap) => {
      const array = wrap([
        { name: "Alice", age: 25 },
        { name: "Bob", age: 30 },
      ...
      expect(out).toEqual({
        resultSchema: [undefined, undefined, undefined],
        resultModel: [undefined, undefined, undefined],
      });
    },
  );

  it.each(shapes)(
    "wraps extracted value in targetPath key (%s input)",
    async (_label, wrap) => {
      const array = wrap([
        { name: "Alice", age: 25 },
        { name: "Bob", age: 30 },
  ...
  it.each(shapes)(
    "wraps extracted value in nested targetPath (%s input)",
    async (_label, wrap) => {
      const array = wrap([{ name: "Alice" }, { name: "Bob" }]);

      const out = await mapArray({
        array,
        path: "name",
        targetPath: "user.label",
      });
  ...
  it.each(shapes)(
    "wraps nested path extraction in targetPath (%s input)",
    async (_label, wrap) => {
      const array = wrap([
        { user: { name: "Alice" } },
        { user: { name: "Bob" } },
  ...
  describe("required option validation", () => {
    ...
    it("throws when 'path' is undefined", async () => {
      const array = { data: [{ name: "Alice" }] };
      await expect(mapArray({ array, path: undefined })).rejects.toThrow(
        "Array Map: 'path' is required!",
      );
    });
  });
  ...
  describe("other edge cases", () => {
    it.each(shapes)(
      "maps over an array of primitives using a non-dot path key (%s input)",
      async (_label, wrap) => {
    ...
    it("throws when a nested path is used but an item is a primitive, not an object", async () => {
      ...
      await expect(mapArray({ array, path: "user.name" })).rejects.toThrow(
