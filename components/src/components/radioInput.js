(() => ({
  ...
  jsx: (() => {
    ...
    const parsedManualValues = useText(manualValues)
      .split('\n')
      .filter(Boolean);
    const {
    ...
    const isDev = env === 'dev';
    const modelProperty =
      (optionType === 'model' && getProperty(property || '')) || {};
    ...
    if (!isListProperty && !isDev && optionType !== 'manual') {
      if (!modelId) {
    ...
    const renderRadios = () => {
      ...
      if (!loading && !isDev) {
        ...
        return results.map((result) =>
          renderRadio(result.id, result[labelKey]),
        );
      }
